...
function selectInviteRole(el,role){
  ...
  if(role==='child'){
    ...
    if(emailSection)emailSection.style.display='none';
    if(pinSection)pinSection.style.display='block';
    if(inviteBtn)inviteBtn.textContent='Create guest account';
    if(inviteSub)inviteSub.textContent='Guest gets PIN login. Set their calendar restrictions after you save.';
  } else {
...
/* Save exam */
function _getExamFields(){
  var titleEl=document.querySelector('#modal-add-exam input[type=text]');
  var dateEl=document.querySelector('#modal-add-exam input[type=date]');
  var subjectEl=document.querySelector('#modal-add-exam select');
  var notesEl=document.querySelector('#modal-add-exam input[placeholder*="Chapter"]');
  var assigneePills=document.querySelectorAll('#modal-add-exam .role-pill.sel');
  var assignees=[];
  assigneePills.forEach(function(p){assignees.push(p.textContent.trim());});
  if(!assignees.length)assignees=['Everyone'];
  return {
...
function saveExamOnly(){
  var f=_getExamFields();
  if(!f.title){showToast('Enter exam title');return;}
  if(!f.date){showToast('Enter exam date');return;}
  fetch('/api/entries',{method:'POST',headers:{'Content-Type':'application/json'},
    body:JSON.stringify({title:f.title,date:f.date,type:'exam',colour:'red',assignees:f.assignees,recur:'none',subject:f.subject,notes:f.notes||null})
  }).then(function(r){
    if(r.ok){closeModal('modal-add-exam');showToast('Exam added');}
    else{showToast('Could not save exam');}
  }).catch(function(){showToast('Could not save exam');});
}
function saveExamThenRevise(){
  var f=_getExamFields();
  if(!f.title){showToast('Enter exam title');return;}
  if(!f.date){showToast('Enter exam date');return;}
  fetch('/api/entries',{method:'POST',headers:{'Content-Type':'application/json'},
    ...
    if(!r.ok){showToast('Could not save exam');return;}
    showToast('Exam saved — now add revision sessions');
    closeModal('modal-add-exam');
    // Pre-fill revision modal
    var revTitle=document.getElementById('rev-title');
    var revDate=document.getElementById('rev-date');
    var revEndDate=document.getElementById('rev-end-date');
    if(revTitle)revTitle.value=f.title+' revision';
    if(revEndDate)revEndDate.value=f.date;
    // Pre-select same assignees in revision modal
    var revPills=document.querySelectorAll('#rev-assignees .role-pill');
    revPills.forEach(function(p){
      p.classList.remove('sel');
      if(f.assignees.includes(p.textContent.trim()))p.classList.add('sel');
    });
    setTimeout(function(){openModal('modal-add-revision');},200);
  }).catch(function(){showToast('Could not save exam');});
}
function saveRevision(){
  ...
  var endDateEl=document.getElementById('rev-end-date');
  var title=titleEl&&titleEl.value.trim();
  var date=dateEl&&dateEl.value;
  var endDate=endDateEl&&endDateEl.value;
  if(!title){showToast('Enter a topic');return;}
  if(!date){showToast('Enter first session date');return;}
  var dayPills=document.querySelectorAll('#rev-days .role-pill.sel');
  var days=[];
  dayPills.forEach(function(p){days.push(p.textContent.trim());});
  if(!days.length)days=['Mon','Wed'];
  var assigneePills=document.querySelectorAll('#rev-assignees .role-pill.sel');
  var assignees=[];
  assigneePills.forEach(function(p){assignees.push(p.textContent.trim());});
  if(!assignees.length)assignees=['Everyone'];
  fetch('/api/entries',{method:'POST',headers:{'Content-Type':'application/json'},
    body:JSON.stringify({title:title,date:date,time:timeEl&&timeEl.value||null,type:'revision',colour:'purple',assignees:assignees,
      recur:'weekly',recurDays:days,recurEnd:endDate?'on':'never',recurEndDate:endDate||null})
  }).then(function(r){
    if(r.ok){closeModal('modal-add-revision');showToast('Revision sessions added ✓');}
    else{showToast('Could not save revision');}
  }).catch(function(){showToast('Could not save revision');});
}
